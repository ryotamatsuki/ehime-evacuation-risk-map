...
const METRICS: Record<MetricId, { label: string; unit: string; note: string }> = {
  walking: { label: '避難時間', unit: '分', note: '選択中の歩行シナリオで避難場所までに要する時間' },
  tsunami: { label: '津波浸水曝露', unit: '%', note: '500mメッシュ内の津波浸水サンプル割合' },
  aging65: { label: '65歳以上人口割合', unit: '%', note: '2020年国勢調査500mメッシュ' },
  route: { label: '経路津波曝露', unit: '%', note: '避難経路のうち津波浸水想定域を通る割合' },
  score: { label: '政策優先度', unit: '点', note: '探索的な5要素合成指標。公式基準ではありません' },
  capacity: { label: '収容負荷', unit: '%', note: 'STEP 4で避難場所単位に再集計するまで参考表示' },
}
...
const capacityPressure = (row: JsonRecord | undefined | null): number | null => {
  ...
  return toNumber(row, 'capacity_pressure')
}
...
const reasonRows = (row: RiskRow, mode: ModeId): Reason[] => {
  ...
  const route = toNumber(row, 'route_inundation_ratio')
...
function App() {
  ...
  const selectedShelterId = selectedRow ? toText(selectedRow, 'selected_shelter_common_id') ?? toText(selectedRow, 'nearest_shelter_id') : null
  ...
  const mapData = useMemo<GeoCollection>(() => ({
    ...
    features: populationFeatures.map((feature) => {
      ...
      if (risk) {
        ...
        properties.tsunami_pct = (toNumber(risk, 'tsunami_inundation_ratio') ?? 0) * 100
        properties.aging65_pct = (toNumber(risk, 'aging_rate_65plus') ?? 0) * 100
        properties.route_pct = (toNumber(risk, 'route_inundation_ratio') ?? 0) * 100
        ...
        properties.analysis_target = (toNumber(risk, 'tsunami_inundation_ratio') ?? 0) > 0
      }
  ...
  const summary = useMemo(() => {
    const tsunamiProxyPopulation = filteredRows.reduce((sum, row) => sum + (toNumber(row, 'total_population') ?? 0) * (toNumber(row, 'tsunami_inundation_ratio') ?? 0), 0)
    ...
    const routeGaps = filteredRows.filter((row) => toText(row, 'route_status') === 'network_coverage_gap' || walkingMinutes(row, '1.0') === null)
    ...
    if (capacityTrusted) {
      for (const row of filteredRows) {
        ...
        const id = toText(row, 'selected_shelter_common_id') ?? toText(row, 'nearest_shelter_id')
        if (pressure !== null && pressure > 1 && id) overloaded.add(id)
      }
    }
    return {
      ...
      networkGapPopulation: populationFor(routeGaps),
  ...
  const selectedDistance = selectedRow ? toNumber(selectedRow, 'total_walking_distance_m') ?? toNumber(selectedRow, 'network_distance_m') : null
  const selectedNetworkDistance = selectedRow ? toNumber(selectedRow, 'network_path_distance_m') ?? toNumber(selectedRow, 'network_distance_m') : null
  ...
  useEffect(() => {
    ...
    map.setLayoutProperty('route-line-casing', 'visibility', showRoute && selectedRow ? 'visible' : 'none')
    ...
    <div className="app-shell">
      <header className="app-header">
        ...
        <div className="header-actions">
          <div className="header-status"><span className="status-dot" /><b>{loading ? 'データ読込中' : '14市町'}</b><span>/ {formatNumber(filteredRows.length || 1090)}津波対象メッシュ</span></div>
      ...
      <section className="kpi-strip" aria-label="主要指標">
        ...
        <KpiCard icon="warning" tone="red" label="network gap人口" value={`${formatNumber(summary.networkGapPopulation)}人`} sub="経路なし・coverage gap" />
        <KpiCard icon="shelter" tone="purple" label="容量超過避難場所" value={capacityTrusted ? `${formatNumber(summary.overloadedShelters)}施設` : '再計算待ち'} sub={capacityTrusted ? '避難需要 / 収容人数 > 100%' : 'Analysis Core STEP 4'} muted={!capacityTrusted} />
      </section>
        ...
        <section className="map-panel" aria-label="避難時間マップ">
          ...
          <div className="map-control layer-card">
            ...
            <select value={metric} onChange={(event) => setMetric(event.target.value as MetricId)}>
              ...
              <option value="capacity" disabled={!capacityTrusted}>収容負荷{!capacityTrusted ? '（再計算待ち）' : ''}</option>
            </select>
          </div>
          ...
          <div className="diagnostic-scroll">
            ...
            {selectedRow ? <>
              <section className="hero-diagnosis">
                ...
                <div className="fact-grid">
                  <Fact icon="school" label="最寄り避難場所" value={toText(selectedShelter, 'name') ?? selectedShelterId ?? '—'} />
                  <Fact icon="distance" label="総距離" value={formatMeters(selectedDistance)} />
                  <Fact icon="walk" label="道路まで" value={selectedOriginConnector === null ? '現行データ未分離' : formatMeters(selectedOriginConnector)} />
                  <Fact icon="route" label="道路上" value={formatMeters(selectedNetworkDistance)} />
                  <Fact icon="distance" label="避難場所入口まで" value={selectedShelterConnector === null ? '現行データ未分離' : formatMeters(selectedShelterConnector)} />
                  <Fact icon="wave" label="経路の津波曝露" value={formatPercent(toNumber(selectedRow, 'route_inundation_ratio'))} />
                  <Fact icon="people" label="避難場所収容負荷" value={capacityTrusted ? formatPct100((capacityPressure(selectedRow) ?? 0) * 100) : 'STEP 4再計算待ち'} alert={capacityTrusted && (capacityPressure(selectedRow) ?? 0) > 1} />
                  <Fact icon="database" label="データ完全度" value={formatPercent(toNumber(selectedRow, 'data_completeness'))} />
                </div>
              </section>
            ...
            <details className="advanced-card">
              ...
              <div className="advanced-body">
                <div className="advanced-note">政策優先度の探索的重み。公式な政策基準ではありません。</div>
      ...
      <footer className="disclaimer"><span>本サイトは公開データを用いた政策分析・可視化PoCです。実際の避難行動や避難経路の安全を保証するものではありません。</span><span>人口：2020年国勢調査 / 津波：ハザードマップポータル / 避難場所：愛媛県・国土地理院 / 道路：OpenStreetMap</span></footer>
    </div>
  )
}
