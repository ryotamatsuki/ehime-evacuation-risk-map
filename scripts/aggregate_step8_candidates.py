...
Rank 1 is checked against both STEP 2 rebuilt on the identical GraphML and the
immutable corrected public Analysis Core v4 artifact. Only after both route
identity/distance gates pass is rank-1 municipality/cross-border metadata
anchored to the corrected public artifact. Ranks 2+ retain route-run metadata.
"""
...
def load_canonical_routes(public_data_root: pathlib.Path) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    for item in index:
        ...
        payload = json.loads((public_data_root / relative).read_text(encoding="utf-8"))
        if not isinstance(payload, list):
            raise ValueError(f"canonical route payload is not a list: {relative}")
        rows.extend(payload)
...
def _as_bool(series: pd.Series) -> pd.Series:
    ...
    return series.astype("string").fillna("false").str.lower().eq("true")
...
def aggregate_step8_candidates(
    ...
    *,
    ...
    canonical_routes = canonical_routes.copy()
    ...
    if status["mesh_id"].nunique() != len(status):
        failures.append("duplicate mesh_id in candidate status")
    ...
    if set(baseline["mesh_id"]) != mesh_set:
        failures.append("candidate status mesh set differs from same-graph STEP 2 baseline")
    if set(canonical_routes["mesh_id"]) != mesh_set:
        failures.append("candidate status mesh set differs from canonical public routes")
    ...
    if not canonical_contract:
        failures.append("canonical Analysis Core v4 metadata contract mismatch")
    ...
    if int((~complete_status).sum()) != expected_rows - expected_complete:
        failures.append("candidate failure count differs from expected")

    same_status = status.merge(baseline[["mesh_id", "route_status"]], on="mesh_id", how="left", validate="one_to_one")
    same_status_mismatch = same_status["candidate_status"].astype(str) != same_status["route_status"].astype(str)
    if same_status_mismatch.any():
        failures.append(f"candidate/same-graph STEP2 route status mismatch rows={int(same_status_mismatch.sum())}")
    canonical_status = status.merge(canonical_routes[["mesh_id", "route_status"]], on="mesh_id", how="left", validate="one_to_one")
    canonical_status_mismatch = canonical_status["candidate_status"].astype(str) != canonical_status["route_status"].astype(str)
    if canonical_status_mismatch.any():
        failures.append(f"candidate/canonical route status mismatch rows={int(canonical_status_mismatch.sum())}")
    ...
    if len(candidates):
        ...
        candidates["shelter_municipality_code"] = candidates["shelter_municipality_code"].astype("string")
        if candidates[["candidate_rank", "total_walking_distance_m"]].isna().any().any():
            failures.append("candidate rows contain invalid rank or distance")
        if candidates.duplicated(["mesh_id", "shelter_key"]).any():
            failures.append("duplicate mesh_id/shelter_key candidate rows")
        if candidates.duplicated(["mesh_id", "candidate_rank"]).any():
            failures.append("duplicate candidate rank within a mesh")
        if candidates["candidate_rank"].lt(1).any() or candidates["candidate_rank"].gt(candidate_limit).any():
            failures.append("candidate rank outside configured limit")
        for mesh_id, group in candidates.groupby("mesh_id", sort=False):
            ordered = group.sort_values("candidate_rank")
            ranks = [int(value) for value in ordered["candidate_rank"]]
            if ranks != list(range(1, len(ranks) + 1)):
                failures.append(f"non-consecutive candidate ranks for mesh {mesh_id}")
                break
            distances = ordered["total_walking_distance_m"].astype(float).to_numpy()
            if len(distances) > 1 and np.any(np.diff(distances) < -1e-7):
                failures.append(f"candidate distance order violation for mesh {mesh_id}")
                break

    rank1 = candidates.loc[candidates["candidate_rank"].eq(1)].copy() if len(candidates) else candidates.copy()
    if len(rank1) != expected_complete:
        failures.append(f"rank1 candidate rows={len(rank1)} expected={expected_complete}")

    same_complete = baseline.loc[baseline["route_status"].astype(str).eq("complete")].copy()
    same_compare = same_complete.merge(
    ...
    same_identity = _identity_mask(
        same_compare["selected_shelter_common_id"], same_compare["selected_shelter_name"],
        same_compare["shelter_common_id"], same_compare["shelter_name"],
    )
    if not bool(same_identity.all()):
        failures.append(f"same-graph rank1 shelter identity mismatch rows={int((~same_identity).sum())}")
    same_delta, same_max_delta = _distance_delta(
        same_compare["total_walking_distance_m_step2"], same_compare["total_walking_distance_m_step8"]
    )
    if same_delta.isna().any() or (same_delta > DISTANCE_TOLERANCE_M).any():
        failures.append(f"same-graph rank1 distance mismatch; max absolute delta={same_max_delta}")
    same_cross = int(_as_bool(same_compare["cross_border_step8"]).sum())
    ...
    if not bool(canonical_identity.all()):
        failures.append(f"canonical rank1 shelter identity mismatch rows={int((~canonical_identity).sum())}")
    canonical_delta, canonical_max_delta = _distance_delta(
        canonical_compare["total_walking_distance_m_canonical"], canonical_compare["total_walking_distance_m_step8"]
    )
    ...
    raw_cross = _as_bool(canonical_compare["cross_border_step8"])
    canonical_cross_mask = _as_bool(canonical_compare["cross_border_canonical"])
    canonical_cross = int(canonical_cross_mask.sum())
    metadata_corrections = int((raw_cross != canonical_cross_mask).sum())
    correction_meta = canonical_metadata.get("cross_border_metadata_correction", {})
    if not isinstance(correction_meta, dict):
        correction_meta = {}
    expected_metadata_corrections = int(correction_meta.get("additional_cross_border_detected", 0) or 0)
    if canonical_cross != EXPECTED_CANONICAL_CROSS_BORDER:
        failures.append(f"canonical rank1 cross-border routes={canonical_cross} expected={EXPECTED_CANONICAL_CROSS_BORDER}")
    if metadata_corrections != expected_metadata_corrections:
        failures.append(
            "rank1 canonical metadata correction count differs from final-export metadata: "
            f"observed={metadata_corrections} expected={expected_metadata_corrections}"
        )

    route_parity_pass = (
        bool(same_identity.all()) and not same_delta.isna().any() and not (same_delta > DISTANCE_TOLERANCE_M).any()
    ...
    if route_parity_pass:
        anchor = canonical_complete.set_index("mesh_id")
        for idx in candidates.index[candidates["candidate_rank"].eq(1)]:
            canonical_row = anchor.loc[str(candidates.at[idx, "mesh_id"])]
            candidates.at[idx, "shelter_municipality_code"] = str(canonical_row["shelter_municipality_code"])
    ...
    if anchored_cross != EXPECTED_CANONICAL_CROSS_BORDER:
        failures.append(f"anchored rank1 cross-border routes={anchored_cross} expected={EXPECTED_CANONICAL_CROSS_BORDER}")

    counts = status["candidate_count"].fillna(0).astype(int)
    qa = {
        "step": "STEP 8B - candidate routing double-parity release gate",
        "target_meshes": int(len(status)),
        "complete_candidate_meshes": int(complete_status.sum()),
        "route_unavailable": int((~complete_status).sum()),
        "candidate_limit": int(candidate_limit),
        "candidate_route_rows": int(len(candidates)),
        "rank1_rows": int(len(rank1)),
        "same_graph_rank1_identity_matches": int(same_identity.sum()) if len(same_compare) else 0,
        "same_graph_rank1_max_distance_delta_m": same_max_delta,
        "same_graph_source_cross_border_routes": same_cross,
        ...
        "canonical_source_workflow_run_id": canonical_metadata.get("workflow_run_id"),
        ...
        "candidate_capacity_filtering": False,
        "canonical_step2_modified": False,
        "release_gate": "PASS" if not failures else "FAIL",
        "failures": failures,
    }
    return (
        candidates.sort_values(["mesh_id", "candidate_rank"]).reset_index(drop=True),
        status.sort_values("mesh_id").reset_index(drop=True),
        qa,
        failures,
    )
...
def main() -> None:
    ...
    parser.add_argument("--baseline-routes-csv", type=pathlib.Path, required=True)
    parser.add_argument("--canonical-public-data-root", type=pathlib.Path, required=True)
    parser.add_argument("--out-candidates-csv", type=pathlib.Path, required=True)
    parser.add_argument("--out-status-csv", type=pathlib.Path, required=True)
    ...
    parser.add_argument("--expected-complete", type=int, default=EXPECTED_COMPLETE_ROUTES)
    ...
    candidates_out, status_out, qa, failures = aggregate_step8_candidates(
        candidates, status, baseline, canonical_routes, canonical_metadata,
        expected_rows=args.expected_rows, expected_complete=args.expected_complete, candidate_limit=args.candidate_limit,
    )
    args.out_candidates_csv.parent.mkdir(parents=True, exist_ok=True)
    args.out_status_csv.parent.mkdir(parents=True, exist_ok=True)
    args.out_qa.parent.mkdir(parents=True, exist_ok=True)
    candidates_out.to_csv(args.out_candidates_csv, index=False, encoding="utf-8-sig")
    status_out.to_csv(args.out_status_csv, index=False, encoding="utf-8-sig")
    args.out_qa.write_text(json.dumps(qa, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(qa, ensure_ascii=False, indent=2))
    if failures:
        raise SystemExit("STEP 8B release gate failed: " + "; ".join(failures))
